...
class ChangePasswordVC: UIViewController {
    ...
    func submitAction(){
        ...
        user?.reauthenticate(with: credential) { error in
            if let error = error {
                ...
                user?.updatePassword(self.emailTF.text!, completion: {(error) in
                    self.submitButton.backgroundColor = UIColor.clear
                    self.submitButton.setTitleColor(UIColor.black, for: .normal)
                    self.submitButton.setTitle("Password changed!", for: .normal)
    ...
    func setUpButton(){
        ...
        submitButton.addTarget(self, action: #selector(ChangePasswordVC.submitAction), for: .touchUpInside)
